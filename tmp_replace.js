...
const newEn = `    // 2. Bengali-to-English location name mapping for language toggle
    const LOCATION_EN = {
        'আমতলী': 'Amtali',
...
const oldStart = "    // 2. Bengali-to-English location name mapping for language toggle";
const oldEnd = "        'হোসেনপুর (চাঁদপুর)': 'হোসেনপুর (চাঁদপুর)',\n    };";

const idx = content.indexOf(oldEnd);
if (idx === -1) { console.log("ERROR: oldEnd not found"); process.exit(1); }

const afterEnd = content.indexOf("\n\n    // Build reverse map", idx);
if (afterEnd === -1) { console.log("ERROR: after marker not found"); process.exit(1); }

const before = content.substring(0, content.indexOf(oldStart));
const after = content.substring(afterEnd + 1);

fs.writeFileSync('script.js', before + newEn + after);
